...
describe('gua commentary registry', () => {
  it('covers the refreshed gua key set with the known gaps', () => {
    const keys = Object.keys(GUA_COMMENTARY_BY_NUM)
      .map((key) => Number(key))
      .sort((left, right) => left - right);

    expect(keys).toEqual(
      Array.from({ length: 60 }, (_, index) => index + 5).filter((num) => num !== 11 && num !== 28),
    );
    expect(Object.prototype.hasOwnProperty.call(GUA_COMMENTARY_BY_NUM, 5)).toBe(true);
    expect(Object.prototype.hasOwnProperty.call(GUA_COMMENTARY_BY_NUM, 11)).toBe(false);
    expect(Object.prototype.hasOwnProperty.call(GUA_COMMENTARY_BY_NUM, 28)).toBe(false);
