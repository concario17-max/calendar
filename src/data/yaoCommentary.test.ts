...
describe('yao commentary registry', () => {
  it('includes entries from both source files in folder order', () => {
    ...
    expect(Object.prototype.hasOwnProperty.call(YAO_COMMENTARY_BY_NUM, 289)).toBe(true);
  });
  ...
  it('keeps the refreshed fourth-bundle commentary readable', () => {
    const commentary = getYaoCommentary(289);

    expect(commentary).toBeDefined();
    expect(commentary?.startsWith('289. ')).toBe(true);
    expect(commentary).toContain('[[list]]');
  });
