# 핵심 로직 복구 및 시스템 정상화 계획 (Implementation Plan)
...
- [ ] `src/utils/textUtils.test.ts`의 `generateGuidedQuestion` 관련 런타임 에러 픽스용 테스트 보강.
...
- [/] [MODIFY] [logic.ts](file:///c:/Users/PT/Desktop/calendar/src/utils/logic.ts)
    - 모든 유틸리티 함수의 중앙 집중형 export 구조 재정립.

### Phase 4: 검증 및 동기화 (REFACTOR & PUSH)
- [/] `npm test`를 통한 전수 테스트 통과 확인.
- [ ] `useCalendarLogic.ts`의 타입 에러 소거 확인.
- [ ] Ray 표준에 따른 자동 Git Commit/Push 실행.

## 3. 검증 계획
### 자동화 테스트
- `vitest --run` 명령어 실행을 통해 모든 유틸리티 함수의 입출력 정확도 검증.
- 특히 3월 25일(시작일)과 3월 24일(종료일)의 경계값 인덱싱 집중 테스트.

### 수동 확인 (요청 시)
- 브라우저 상에서 날짜 변경 시 Gua/Yao 데이터가 에러 없이 렌더링되는지 확인.
